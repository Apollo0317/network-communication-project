...
class Modulator:
    def __init__(self, scheme: str, symbol_rate: int, sample_rate: int, fc: int, power_factor:int=100):
        self.scheme = scheme
    ...
    def QAM(self, byte_data: bytes) -> list:
        ...
        if not self.has_estimated:
            ...
            print(f'first send: add 4 symbol to estimate alpha\ntotal symbol:{len(symbols)}')
        return symbols
    ...
    def modulate(self, data: bytes) -> np.ndarray:
        ...
        signal=signal*self.power_factor
        return signal
...
class DeModulator:
    ...
    def Detect_Symbol(self, symbols: list[list]) -> list[int]:
        ...
        for symbol in symbols:
            fixed_I, fixed_Q= symbol[0]*self.aplitude_loss, symbol[1]*self.aplitude_loss
...
def test():
    modulator = Modulator(scheme="16QAM", symbol_rate=1e6, sample_rate=50e6, fc=2e6, power_factor=100)
    demodulator = DeModulator(scheme="16QAM", symbol_rate=1e6, sample_rate=50e6, fc=2e6, power_factor=100)
    ...
    cable = Cable(
        ...
        attenuation=1,  # Attenuation coefficient
    ...
    print(f"{len(test_str) / 1000 / cost} KBps")
    pass
